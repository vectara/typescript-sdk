...
import { ClientOptions, isApiKeyOptions, isOAuthOptions } from "ClientOptions";
...
export class VectaraClient {
    ...
    constructor(protected readonly _options: VectaraClient.Options) {
        const _headers: Record<string, core.Supplier<string>> = {
        ...
        if (isApiKeyOptions(_options)) {
            _headers["x-api-key"] = _options.apiKey;
        } else if (isOAuthOptions(_options)) {
            const oauthTokenProvider = new core.OAuthTokenProvider({
            ...
            _headers["Authorization"] = oauthTokenProvider.getToken
        } else {
            throw new Error("Either an API key or client ID and client secret must be provided");
        }
        this._client = new APIClient(_headers);
    }

    public get corpora(): Corpora {
        return (this._corpora ??= new Corpora(this._client));
    }

    public get upload(): Upload {
        return (this._upload ??= new Upload(this._client));
    }

    public get documents(): Documents {
        return (this._documents ??= new Documents(this._client));
    }

    public get index(): Index {
        return (this._index ??= new Index(this._client));
    }

    public get chats(): Chats {
        return (this._chats ??= new Chats(this._client));
    }

    public get llms(): Llms {
        return (this._llms ??= new Llms(this._client));
    }

    public get generationPresets(): GenerationPresets {
        return (this._generationPresets ??= new GenerationPresets(this._client));
    }

    public get encoders(): Encoders {
        return (this._encoders ??= new Encoders(this._client));
    }

    public get rerankers(): Rerankers {
        return (this._rerankers ??= new Rerankers(this._client));
    }

    public get jobs(): Jobs {
        return (this._jobs ??= new Jobs(this._client));
    }

    public get users(): Users {
        return (this._users ??= new Users(this._client));
    }

    public get apiKeys(): ApiKeys {
        return (this._apiKeys ??= new ApiKeys(this._client));
    }

    public get appClients(): AppClients {
        return (this._appClients ??= new AppClients(this._client));
    }

    public get queryHistory(): QueryHistory {
        return (this._queryHistory ??= new QueryHistory(this._client));
    }

    public get auth(): Auth {
        return (this._auth ??= new Auth(this._client));
    }
